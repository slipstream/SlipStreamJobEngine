...
class VirtualMachinesCleanupJob(object):
    ...
    def cleanup_jobs(self):
        self.logger.info(self.thread_log_fn('Cleanup of virtual machines started.'))
        ...
        self.logger.info(self.thread_log_fn(
            'Number of virtual machines to be cleaned up: {}'.format(vms_not_updated.count)))

        vms_deleted = []
        for vm in vms_not_updated.resources_list:
            self.logger.debug(self.thread_log_fn('Cleanup of virtual machine {}.'.format(vm.json.get('id'))))
